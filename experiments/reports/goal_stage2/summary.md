...
本批是公开代码基线模块对照，不是完整严格的设计稿：upstream_sampled/counts 不满足原 docx 的 Top-Z 与 spot/gene Z 基底要求。这里只核验已记录参数，哈希相同不代表参数一致，也不证明 docx 细节逐字一致。设计要求的 C-A 见 paired_vs_formal_a；final-initial 是项目额外验收，不能替代 C-A。最终 predict 与末次 refresh 的置信比例分别记录，不可混用。
...
- A 来源：['D:\\Spatial-scFormer\\experiments\\runs\\stage1_A_seed0_design_v1\\metrics.json', 'D:\\Spatial-scFormer\\experiments\\runs\\stage1_A_seed1_design_v1\\metrics.json', 'D:\\Spatial-scFormer\\experiments\\runs\\stage1_A_seed42_design_v1\\metrics.json']
